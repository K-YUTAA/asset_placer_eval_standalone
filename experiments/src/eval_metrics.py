...
def _collect_occupancy_objects(
    layout: Dict[str, Any],
...
def _build_room_and_occupancy(
    ...
    occupancy_objects = _collect_occupancy_objects(layout, occupancy_exclude_categories)
...
def _build_object_id_grid(
    ...
    ny = len(room_mask)
    nx = len(room_mask[0]) if ny > 0 else 0
    object_id_grid: List[List[int]] = [[0 for _ in range(nx)] for _ in range(ny)]

    occupancy_objects = _collect_occupancy_objects(layout, occupancy_exclude_categories)
    id_to_meta: Dict[int, Dict[str, Any]] = {}
    for grid_id, obj in enumerate(occupancy_objects, start=1):
        obj_id = str(obj.get("id") or f"obj_{grid_id:02d}")
        category = str(obj.get("category") or "").strip().lower() or "unknown"
        id_to_meta[grid_id] = {"id": obj_id, "category": category}
    ...
    for iy in range(ny):
        for ix in range(nx):
            ...
            for grid_id, obj in enumerate(occupancy_objects, start=1):
                if point_in_obb(x, y, obj):
                    object_id_grid[iy][ix] = grid_id
                    break

    return object_id_grid, id_to_meta


def _collect_object_boundary_cells(
    object_id_grid: List[List[int]],
    room_mask: Grid,
) -> Dict[int, List[Cell]]:
    ny = len(room_mask)
    nx = len(room_mask[0]) if ny > 0 else 0
    boundary_by_obj: Dict[int, List[Cell]] = {}

    for iy in range(ny):
        for ix in range(nx):
            obj_id = int(object_id_grid[iy][ix]) if iy < len(object_id_grid) and ix < len(object_id_grid[iy]) else 0
            if obj_id <= 0:
                continue
            is_boundary = False
            for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                ...
                if not room_mask[ty][tx]:
                    is_boundary = True
                    break
                if int(object_id_grid[ty][tx]) != obj_id:
                    is_boundary = True
                    break
            if is_boundary:
                boundary_by_obj.setdefault(obj_id, []).append((ix, iy))

    return boundary_by_obj


def _build_visible_object_map(
    room_mask: Grid,
    object_id_grid: List[List[int]],
    boundary_by_obj: Dict[int, List[Cell]],
    visible_free: Grid,
    ...
    # 0: outside room, 30: room floor/background, 90: object not visible, 255: visible object boundary.
    vis_img: List[List[int]] = [[0 for _ in range(nx)] for _ in range(ny)]
    for iy in range(ny):
        for ix in range(nx):
            if not room_mask[iy][ix]:
                continue
            vis_img[iy][ix] = 30 if int(object_id_grid[iy][ix]) <= 0 else 90

    per_object: Dict[int, Dict[str, Any]] = {}
    for obj_grid_id, boundary_cells in boundary_by_obj.items():
        visible_boundary = 0
        for ix, iy in boundary_cells:
            visible = False
            for tx, ty in _neighbors4(ix, iy, nx, ny):
                if bool(visible_free[ty][tx]):
                    visible = True
                    break
            if visible:
                vis_img[iy][ix] = 255
        ...
        ratio = (visible_boundary / float(boundary_count)) if boundary_count > 0 else 0.0
        meta = id_to_meta.get(obj_grid_id, {})
        per_object[obj_grid_id] = {
            "id": str(meta.get("id") or f"obj_{obj_grid_id:02d}"),
            "category": str(meta.get("category") or "unknown"),
            "boundary_cells": int(boundary_count),
            "visible_boundary_cells": int(visible_boundary),
            "visible_boundary_ratio": ratio,
        }

    return vis_img, per_object
...
def _save_object_visibility_png(
    path: pathlib.Path,
    values: List[List[int]],
    bounds: Tuple[float, float, float, float],
    title: str,
    ...
    rgba[room_floor] = [0.60, 0.60, 0.60, 0.18]
    rgba[hidden_obj] = [0.80, 0.18, 0.18, 0.50]
    rgba[visible_obj] = [0.00, 0.72, 0.22, 0.90]
...
def evaluate_layout(
    ...
    object_id_grid, object_id_meta = _build_object_id_grid(
        layout=layout,
        bounds=bounds,
        resolution=resolution,
        room_mask=room_mask,
        occupancy_exclude_categories=occupancy_exclude_categories,
    )
    object_boundary_by_id = _collect_object_boundary_cells(object_id_grid, room_mask)
    visible_objects_path, visible_object_path_stats = _build_visible_object_map(
        room_mask=room_mask,
        object_id_grid=object_id_grid,
        boundary_by_obj=object_boundary_by_id,
        visible_free=visible_free_path,
        id_to_meta=object_id_meta,
    )
    visible_objects_start, visible_object_start_stats = _build_visible_object_map(
        room_mask=room_mask,
        object_id_grid=object_id_grid,
        boundary_by_obj=object_boundary_by_id,
    ...
    visible_object_summary: List[Dict[str, Any]] = []
    for grid_id, meta in sorted(object_id_meta.items(), key=lambda item: str(item[1].get("id") or "")):
        path_stats = visible_object_path_stats.get(grid_id, {})
        start_stats = visible_object_start_stats.get(grid_id, {})
        boundary_cells = int(path_stats.get("boundary_cells", start_stats.get("boundary_cells", 0)))
        visible_object_summary.append(
            {
                "id": str(meta.get("id") or f"obj_{grid_id:02d}"),
                "category": str(meta.get("category") or "unknown"),
                "boundary_cells": boundary_cells,
                "visible_boundary_cells_path": int(path_stats.get("visible_boundary_cells", 0)),
                "visible_boundary_ratio_path": as_float(path_stats.get("visible_boundary_ratio"), 0.0),
                "visible_boundary_cells_start": int(start_stats.get("visible_boundary_cells", 0)),
                "visible_boundary_ratio_start": as_float(start_stats.get("visible_boundary_ratio"), 0.0),
            }
        )
    ...
    debug = {
        ...
        "visible_objects_path": visible_objects_path,
...
def _save_debug_maps(debug: Dict[str, Any], out_dir: pathlib.Path) -> None:
    ...
    visible_objects_path: List[List[int]] = debug.get("visible_objects_path") or []
    visible_objects_start: List[List[int]] = debug.get("visible_objects_start") or []
    ...
    if len(visible_objects_path) == ny and ny > 0 and len(visible_objects_path[0]) == nx:
        _grid_to_pgm(out_dir / "c_vis_objects.pgm", visible_objects_path)
    if len(visible_objects_start) == ny and ny > 0 and len(visible_objects_start[0]) == nx:
        _grid_to_pgm(out_dir / "c_vis_start_objects.pgm", visible_objects_start)
    ...
    if isinstance(bounds, (list, tuple)) and len(bounds) >= 4:
        ...
        if len(visible_objects_path) == ny and ny > 0 and len(visible_objects_path[0]) == nx:
            _save_object_visibility_png(
                ...
                visible_objects_path,
                bbox,
                "Visible furniture boundary (path sensors)",
            )
        if len(visible_objects_start) == ny and ny > 0 and len(visible_objects_start[0]) == nx:
            _save_object_visibility_png(
                out_dir / "c_vis_start_objects_area.png",
                visible_objects_start,
                bbox,
                "Visible furniture boundary (start only)",
            )
    ...
    if debug.get("visible_object_summary") is not None:
        write_json(out_dir / "c_vis_objects.json", {"objects": debug.get("visible_object_summary")})
